...
result = df["Column2"].nunique() # tekil bilgi sayısı gelir
result = df["Column2"].value_counts() # kaç değer varsa gelir
result = df["Column1"] * 2
...
result = df.columns
result = len(df.columns) # kaç sütun
result = df.index
result = len(df.index) # kaç satır
result = df.info

result = df.sort_values("Column2")
result = df.sort_values("Column3")
result = df.sort_values("Column3", ascending = False) # azalan
